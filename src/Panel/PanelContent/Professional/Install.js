...
function Install(props) {
  const { vmDetails, overview } = props;

  return (
    <div
      className={`${overview ? "professionalMainPanelCard" : ""} ${
        overview ? "p-3" : "px-5 py-3"
      } mb-5`}
      style={{ direction: Interface.i18n.GetLangDirection() }}
    >
      {overview ? (
        <h5 style={{ borderBottom: "1px solid #aaaaaa80" }}>
          {Interface.i18n.T("Install")}
        </h5>
      ) : undefined}
      <PlanTemplates overview={overview} vmDetails={vmDetails} />
      <CdDrives overview={overview} vmDetails={vmDetails} />
    </div>
  );
}
